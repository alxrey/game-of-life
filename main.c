                    ...
                    int32_t *cellAliveCpt_) 
{
    ...
    *cellAliveCpt_ = 0; // reset of cpt
    ...
    for (line=1; line<(WIDTH-1); line++) {  // parcours line tb y compris zone bleue
        for (column=1; column<(HEIGHT-1); column++) {   // parcours column tb y compris zone bleue
            // à chaque case on compare avec chaque case autour :
            for (frameLine=line-1; frameLine<=line+1; frameLine++) {
                for (frameColumn=column-1; frameColumn<=column+1; frameColumn++) {
                    
                    // Si cellule en vie mais que ce n'est pas la cellule centrale, on ++
                    if ((isCellAlive(cellsA_, frameLine, frameColumn) && (frameLine!=line || frameColumn!=column))) {
                        sumCellAliveFrame++;
                    }
                }
            }
            ...
            if (sumCellAliveFrame==3) {
                setCell(cellsB_, line, column, true, cellAliveCpt_);   // si 3 cell voisines vivantes, elle vit
            } else if (sumCellAliveFrame==2) {
                setCell(cellsB_, line, column, isCellAlive(cellsA_, line, column), cellAliveCpt_); // si 2 cell vivante elle reste
            } else {
                setCell(cellsB_, line, column, false, cellAliveCpt_);   // sinon meurt
            }
                  ...
                  int32_t *cellAliveCpt_) 
{
    ...
    switch (userChoice_) {
        ...
        default:
            puts("Numero de configuration inconnu.");
            break;
    }
